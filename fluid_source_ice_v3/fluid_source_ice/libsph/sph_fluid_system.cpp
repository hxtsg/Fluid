...
FluidSystem::FluidSystem()
{
	...
	pointDistance =  pow(m_pointMass/m_restDensity, 1.f/3.f);
	//Poly6 Kernel
...
void FluidSystem::AddVolumn( void )
{
	float_3 fluid_min = {-20, 0, -10};
	float_3 fluid_max = {20, 20, 10};
	_addFluidVolume( fBox3( fVector3(&fluid_min), fVector3(&fluid_max)), pointDistance/m_unitScale );

}


void FluidSystem::SetFluidPointer( System * p )
{}
...
/*
void FluidSystem:: _computeTemprature(void)
{

	float h2 = m_smoothRadius*m_smoothRadius;
	float temprature_coff_k = 1.0f;  // 粒子的热量扩散系数暂时设置为1
	float epsilon = 1.0f;
	for(unsigned int i=0; i<m_pointBuffer.size(); i++)
	{
		...
		deltaTemprature *= 4 * temprature_coff_k * temprature_coff_k / ( temprature_coff_k + temprature_coff_k );

		pi->temp += deltaTemprature;
	}
...
SPH::System * getGhostSPHSystem(void)
{
	static SPH::GhostFluidSystem ghost_theSystem;
	return &ghost_theSystem;
}